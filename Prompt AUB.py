...
"""
Build improved LLM prompts for AUB (presence vs true absence) using ONLY:
RWQ, ALT, FFP, BIO1.

What this script does
- Reads NETWORK.xlsx
- Builds AUB_BIN from AUB_PREZ / AUB_TRUEABS on labeled rows (drops ambiguous 1/1)
- Computes SAFE aggregate stats + rich quantiles for the 4 predictors
- Computes class balance of AUB_BIN for the labeled set
- Creates an improved generation prompt (over-generate trees, quantile anchoring,
  monotonic priors, strict diversity, schema & validation checklist)
- Creates a second "referee" prompt to clean/repair LLM JSON trees
- Saves both prompts under outputs/:
    - paper_llm_trees_AUB_prompt.txt
    - paper_llm_trees_AUB_referee.txt
...
SPECIES_CODE = "AUB"
SPECIES_LONG = "Austropotamobius bihariensis (AUB, Idle Crayfish)"
...
# Ecological priors for AUB (soft guidance; can be violated if stats contradict)
# Summarized from your notes & attached literature: endemic to Apuseni Mts., cool, clean,
# karstic mountain/sub-mountain streams, drought/heat sensitive, needs persistent flow.
DOMAIN_PRIORS = [
    "Higher ALT is generally favorable up to sub-alpine ranges (mountain/sub-mountain streams).",
    "Lower BIO1 (cooler annual mean temperature) increases presence likelihood; warm conditions reduce it.",
    "RWQ: smaller, less wide rivers/streams (lower RWQ) are often favorable; very high RWQ (broad/low-gradient) reduces presence.",
    "FFP: insufficient flow persistence (very low FFP) reduces presence (drought/ephemeral risk). Extremely high FFP may indicate flood-prone lowlands—also unfavorable.",
    "Avoid overly tight boxes; rules should be coarse and ecologically interpretable.",
]

# Quantiles to report (for threshold anchoring hints)
Q_LIST = [0.05, 0.2, 0.4, 0.5, 0.6, 0.8, 0.95]
...
def build_aub_frame(df: pd.DataFrame) -> pd.DataFrame:
    for col in ["AUB_PREZ", "AUB_TRUEABS"]:
        if col not in df.columns:
            raise RuntimeError(f"Missing column: {col}")
    prez = pd.to_numeric(df["AUB_PREZ"], errors="coerce").fillna(0).astype(int)
    tabs = pd.to_numeric(df["AUB_TRUEABS"], errors="coerce").fillna(0).astype(int)
    mask_labeled = (prez == 1) | (tabs == 1)
    data = df.loc[mask_labeled].copy()

    # drop ambiguous 1/1
    both = (data["AUB_PREZ"].astype(int) == 1) & (data["AUB_TRUEABS"].astype(int) == 1)
    if both.any():
        data = data.loc[~both].copy()

    data[f"{SPECIES_CODE}_BIN"] = (data["AUB_PREZ"].astype(int) == 1).astype(int)
    return data
...
def main():
    ...
    out_gen = outd / "paper_llm_trees_AUB_prompt.txt"
    out_ref = outd / "paper_llm_trees_AUB_referee.txt"
    ...
    print("[tip] In your LLM UI, use a moderate temperature (e.g., 0.7–0.9) and generate multiple batches; then score & select.")
