...
"""
Build improved LLM prompts for AUT (presence vs true absence) using ONLY:
RWQ, ALT, FFP, BIO1.

What this script does
- Reads NETWORK.xlsx
- Builds AUT_BIN from AUT_PREZ / AUT_TRUEABS on labeled rows (drops ambiguous 1/1)
- Computes SAFE aggregate stats + rich quantiles for the 4 predictors
- Computes class balance of AUT_BIN for the labeled set
...
- Saves both prompts under outputs/:
    - paper_llm_trees_AUT_prompt.txt
    - paper_llm_trees_AUT_referee.txt
...
SPECIES_CODE = "AUT"
SPECIES_LONG = "Austropotamobius torrentium (AUT)"
...
# Ecological priors (soft). Tweak freely if you want.
DOMAIN_PRIORS = [
    "Higher ALT generally reduces presence probability (colder, steeper, harsher).",
    "Higher RWQ generally increases presence probability (larger flow, more oxygenation).",
    "Extreme FFP (very low or very high) may reduce presence due to instability or harshness.",
    "BIO1: cooler to mild conditions tend to be favorable; very warm conditions can be unfavorable.",
]
...
def build_aut_frame(df: pd.DataFrame) -> pd.DataFrame:
    for col in ["AUT_PREZ", "AUT_TRUEABS"]:
        if col not in df.columns:
            raise RuntimeError(f"Missing column: {col}")
    prez = pd.to_numeric(df["AUT_PREZ"], errors="coerce").fillna(0).astype(int)
    tabs = pd.to_numeric(df["AUT_TRUEABS"], errors="coerce").fillna(0).astype(int)
    mask_labeled = (prez == 1) | (tabs == 1)
    data = df.loc[mask_labeled].copy()

    # drop ambiguous 1/1
    both = (data["AUT_PREZ"].astype(int) == 1) & (data["AUT_TRUEABS"].astype(int) == 1)
    if both.any():
        data = data.loc[~both].copy()

    data[f"{SPECIES_CODE}_BIN"] = (data["AUT_PREZ"].astype(int) == 1).astype(int)
    return data
...
def safe_feature_stats(df_num: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for c in df_num.columns:
        ...
        if s.empty:
            continue
        qs = s.quantile(Q_LIST + [0.5])  # ensure 0.5 present (in case duplicates)
        stats[c] = {
            "min": float(s.min()),
            "max": float(s.max()),
...
for AUT_BIN using ONLY features: RWQ, ALT, FFP, BIO1. Your task:
...
def main():
    ...
    out_gen = outd / "paper_llm_trees_AUT_prompt.txt"
    out_ref = outd / "paper_llm_trees_AUT_referee.txt"
